      ...
      const createLegacyOutput = (options = {}) => {
        return {
          ...
          entryFileNames: path.posix.join(
            ...
            `legacy-${options.entryFileNames}`
          ),
          chunkFileNames: path.posix.join(
            config.build.assetsDir,
            `legacy-${options.chunkFileNames}`
          )
        }
      }
...
async function buildPolyfillChunk(
  ...
  const chunkName = buildOptions.rollupOptions.output[0].chunkFileNames;
  const res = await build({
    ...
    build: {
      ...
      rollupOptions: {
        input: {
          [name.replace('legacy-', '')]: polyfillId
        },
        output: {
